...
const translateToEnglish = (feedback) => {
  ...
  router.get('/aggregated-ratings', async (req, res) => {
    ...
    try {
        ...
        const positiveCount = await Rating.countDocuments({ eventId: eventObjectId, sentiment: 'positive' });
        const negativeCount = await Rating.countDocuments({ eventId: eventObjectId, sentiment: 'negative' });
        const neutralCount = await Rating.countDocuments({ eventId: eventObjectId, sentiment: 'neutral' });

        const aggregatedRatings = [
            { trait: 'Positive', count: positiveCount },
            { trait: 'Negative', count: negativeCount },
            { trait: 'Neutral', count: neutralCount },
        ];

        return res.status(200).json({ aggregatedRatings });
    } catch (error) {
        console.error('Error processing sentiment counts:', error.message);
