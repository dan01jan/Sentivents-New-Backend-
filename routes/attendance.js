...
router.put('/updateUsersAttendance/:selectedEvent', async (req, res) => {
    try {
      ...
      // Fetch the event to check its dateEnd
      const event = await Event.findById(selectedEvent);
      if (!event) {
        return res.status(404).json({ message: `Event with ID ${selectedEvent} not found` });
      }
  
      const currentDate = new Date();
      const isEventDone = currentDate > new Date(event.dateEnd);
      ...
      for (const user of users) {
        ...
        // If the event is done and the user has not attended, mark them as absent
        if (isEventDone && !hasAttended) {
